---
name: nixos-build
description: Validate NixOS configuration changes in this repo by running `nh os build` and interpreting the errors. Use whenever `.nix` files under `modules/` are created or modified — especially secret file paths, new options/`config` blocks, or package/`perSystem` definitions.
---
...
- **Missing path** (`path '...' does not exist`): a relative path to a secret or file is wrong. Count the directory depth of the referencing `.nix` file relative to the repo root and correct the `../` prefix count so it resolves to the target at the repo root.
...
- `sops.secrets.*` / secret file paths
