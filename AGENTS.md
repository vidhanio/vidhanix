...
- `modules/flake/` — flake-level machinery: `treefmt.nix`, `files/` (generated files: justfile, readme, gitignore, license, workflows, actions), `packages/`, `pre-commit/`, `dev-shell.nix`, `settings.nix`, `nixpkgs.nix`, `sops-nix.nix`, `substituters.nix`.
...
- **Formatting**: treefmt with `on-unmatched = "fatal"` (nixfmt, statix, deadnix, shfmt, shellcheck, stylua, ruff, oxfmt, xmllint, keep-sorted). A new file type needs a formatter entry in `modules/flake/treefmt.nix`, or `just fmt` fails.
...
- `modules/flake/files/workflows/` — the CI workflows (`ci`, `update-packages`, `sync-dependabot`, `watch-github-refs`) and the shared `setup-nix` composite action.
...
- **CI** (workflows generated from `modules/flake/files/workflows/`, one module per workflow): `ci` runs on push and pull requests, checking formatting (`just fmt --ci`) and generated files (`just generate` + `git diff --exit-code`), evaluating every `nixosConfiguration` via `toplevel.drvPath` (no builds), and building every `perSystem.packages.*`. `update-packages` runs daily and on pushes to main: it bumps each package with a `passthru.updateScript` and opens a signed fixed-branch PR via `peter-evans/create-pull-request`. `sync-dependabot` runs `pull_request_target` on every Dependabot PR: its `metadata` job reads the PR with `dependabot/fetch-metadata`, and two follow-up jobs dispatch on the `package-ecosystem` output — `sync-actions` (ecosystem `github_actions`) applies the bumped action refs from the metadata into the workflow modules from a base checkout, `prune-lock` (ecosystem `nix`) checks out the PR head and runs `just generate` to re-prune and regenerate the lock and files. Both commit back to the PR branch with `planetscale/ghcommit-action`; a re-run on an already-synced PR finds no changes and the commit step is a clean no-op, so no sender guard is needed. `watch-github-refs` runs daily (plus manual dispatch) and opens issues for stale upstream refs.
- **Nix on CI**: the shared composite action `files.actions.setup-nix` plants `FONTS_SSH_KEY` in an ssh-agent (the private `vidhan-fonts` input is `git+ssh`), carves runner disk space, installs nix with `cachix/install-nix-action` (no DeterminateSystems actions), and restores/saves the nix store via `nix-community/cache-nix-action` keyed on the `flake.lock` hash — the save is why jobs ask for `actions: write`.
- **Auth**: workflows authenticate as a GitHub App via `actions/create-github-app-token`, with `APP_CLIENT_ID` as an Actions variable and `APP_PRIVATE_KEY` as an Actions secret (the app needs contents and pull-requests write). Dependabot-triggered runs get a read-only `GITHUB_TOKEN` and secrets only from Dependabot secrets (variables still resolve), so `FONTS_SSH_KEY` and `APP_PRIVATE_KEY` must also exist as Dependabot secrets for the sync/prune flows to work.
- **Dependabot** (`.github/dependabot.yaml`, hand-maintained, not generated): opens daily PRs for the `nix` and `github-actions` ecosystems; its own update runs reach the private fonts input through the `registries` entry backed by the `FONTS_PAT` Dependabot secret. `treefmt` `on-unmatched = "fatal"` means a file matching no formatter fails CI.
