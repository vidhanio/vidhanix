...
4. Declare each class inside the aspect, for example `flake.aspects.<name> = { homeManager = ...; };`. Files under `modules/` merge automatically, so do not import repository modules manually. Use `imports` only for modules exposed by flake inputs through outputs such as `nixosModules`, `homeModules`/`homeManagerModules`, or `flakeModules`. Declare new flake inputs as `flake-file.inputs.<name>` in the aspect that consumes them.
